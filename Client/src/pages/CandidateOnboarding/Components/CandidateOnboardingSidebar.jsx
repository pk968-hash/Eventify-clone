...
const steps = [
  { key: "Email", title: "Your email", desc: "Provide your email" },
  {
    key: "AccountDetails",
    title: "Account Details",
    desc: "Setup your account",
  },
  { key: "ID", title: "Your ID", desc: "Enter identity information" },
  {
    key: "Verification",
...
const CandidateOnboardingSidebar = ({ step }) => {
  const currentStepIndex = steps.findIndex((s) => s.key === step);

  return (
    <div className="w-full min-h-screen bg-[#F2F5F8] select-none flex flex-col justify-between fixed">
      {/* Logo */}
          ...
          return (
            <div key={s.key} className="flex items-center gap-2 px-10 mt-6">
              ...
              <div>
                <h3
                  className={`text-[15px] ${
                    isCurrent ? "font-medium text-black" : "font-normal"
                  }`}
                >
                  {s.title}
                </h3>
                <p
                  className={`text-[14px] ${
                    isCurrent ? "font-medium text-gray-700" : "text-gray-500"
                  }`}
                >
                  {s.desc}
                </p>
      ...
      {/* Footer */}
      <div className="flex justify-start py-6 border-t border-gray-300 px-10">
...
export default CandidateOnboardingSidebar;
